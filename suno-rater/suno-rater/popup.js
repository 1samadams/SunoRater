...
let songs = [];
let raw = {}; // id -> record, including deletion markers ({ d: 1, u })
...
function load() {
  chrome.storage.local.get(null, (all) => {
    raw = {};
    for (const [k, v] of Object.entries(all || {})) if (k.startsWith('s:')) raw[k.slice(2)] = v;
    songs = Object.entries(all || {})
      .filter(([k, v]) => k.startsWith('s:') && v && !v.d && (v.r || v.n))
...
$('file').onchange = async (e) => {
  ...
  try {
    ...
    for (const s of incoming) {
      ...
      const cur = raw[s.id.toLowerCase()];
      if (cur && (cur.u || 0) >= (s.u || 0)) continue; // newest edit wins, deletions included
